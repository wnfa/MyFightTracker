...
namespace MyFightTracker.Data
{
    ...
    public class BusinessRulesBase : ActionHandlerBase, MyFightTracker.Data.IRowHandler, MyFightTracker.Data.IDataFilter, MyFightTracker.Data.IDataFilter2
    {
        ...
        {
            DynamicAccessControlList dacl = ((DynamicAccessControlList)(Context.Items["DbDacl"]));
            if (dacl == null)
            {
                dacl = new DynamicAccessControlList();
                ...
                {
                    SiteContentFileList files = ApplicationServices.Current.ReadSiteContent("sys/dacl%", "%");
                    foreach (SiteContentFile f in files)
                    	if (f.Data != null)
        ...
        {
            if (ApplicationServices.IsSiteContentEnabled && controllerName != ApplicationServices.SiteContentControllerName)
            {
                SiteContentFileList alterations = ApplicationServices.Current.ReadSiteContent("sys/controllers%", (controllerName + ".Alter%"));
        ...
        {
            bool changed = false;
            foreach (SiteContentFile f in alterations)
            {
                ControllerNodeSet nodeSet = this.NodeSet();
                string alter = f.Text;
                if (!(String.IsNullOrEmpty(alter)))
                    ...
                    {
                        Match m = AlterMethodRegex.Match(alter);
                        bool skipInvoke = false;
                        while (m.Success)
                        {
                            string method = m.Groups["Method"].Value;
                            string parameters = m.Groups["Parameters"].Value;
                            string terminator = m.Groups["Terminator"].Value;
                            StringBuilder sb = new StringBuilder();
                            foreach (string s in method.Split(new char[] {
                                        '-'}, StringSplitOptions.RemoveEmptyEntries))
                            	sb.Append((Char.ToUpper(s[0]) + s.Substring(1)));
                            method = sb.ToString();
                            List<string> args = new List<string>();
                            Match p = AlterParametersRegex.Match(parameters);
                            while (p.Success)
                            {
                            ...
                            try
                            {
                                ...
                                if (args.Count > 0)
                                {
                                    ...
                                    if (method == "WhenUrl")
                                    {
                                        Regex urlRegex = new Regex(args[0], RegexOptions.IgnoreCase);
                                    ...
                                    {
                                        string userInterface = args[0].ToLower();
                                    ...
                                    if (method == "WhenView")
                                    {
                                        if (!(skipInvoke))
                                        {
                                            Regex viewRegex = new Regex(args[0], RegexOptions.IgnoreCase);
                                    ...
                                    if (method == "WhenSql")
                                    {
                                        if (!(skipInvoke))
                                        {
                                            string q = args[0].Trim();
                                            string sqlText = "select 1";
                                            ConnectionStringSettings css = ConnectionStringSettingsFactory.Create(null);
