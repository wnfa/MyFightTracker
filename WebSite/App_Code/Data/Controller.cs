...
namespace MyFightTracker.Data
{
	public partial class DataControllerBase
    {
        ...
        public static ControllerConfiguration CreateConfigurationInstance(Type t, string controller)
        {
            ...
            if (config == null)
            {
                ...
                if (res == DefaultDataControllerStream)
                	res = null;
                if (res == null)
                	res = t.Assembly.GetManifestResourceStream(String.Format("MyFightTracker.Controllers.{0}.xml", controller));
                if (res == null)
                	res = t.Assembly.GetManifestResourceStream(String.Format("MyFightTracker.{0}.xml", controller));
                if (res == null)
                {
        ...
        {
            foreach (DataField field in page.Fields)
            	if (field.IsPrimaryKey || page.IncludeField(field.Name))
                {
                    if (firstField)
                    	firstField = false;
                    else
        ...
        private void EnsurePageFields(ViewPage page, SelectClauseDictionary expressions)
        {
            ...
            {
                string siteContentTextFieldName = ApplicationServices.Current.SiteContentFieldName(SiteContentFields.Text);
        ...
        public static string CreateReportInstance(Type t, string name, string controller, string view, bool validate)
        {
            ...
            if (t == null)
            	t = typeof(MyFightTracker.Data.Controller);
            Stream res = t.Assembly.GetManifestResourceStream(String.Format("MyFightTracker.Reports.{0}", name));
            if (res == null)
            	res = t.Assembly.GetManifestResourceStream(String.Format("MyFightTracker.{0}", name));
            if (res == null)
            {
            ...
            if (isGeneric)
            {
                ...
                ControllerConfiguration config = MyFightTracker.Data.Controller.CreateConfigurationInstance(t, controller);
