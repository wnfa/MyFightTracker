        ...
        public bool PopulateStaticItems(DataField field, FieldValue[] contextValues)
        {
            ...
            if (field.SupportsStaticItems() && (String.IsNullOrEmpty(field.ContextFields) || (contextValues != null)))
            {
                ...
                try
                {
                    ...
                    PageRequest request = new PageRequest(-1, 1000, sortExpression, filter);
        ...
        public ViewPage ToResult(ControllerConfiguration configuration, XPathNavigator mainView)
        {
            ...
            else
            {
                ...
                if (IncludeMetadata("layouts"))
                {
                    ...
                    string fileName = String.Format("{0}.{1}.html", this.Controller, this.View);
                    bool tryLoad = true;
                    while (tryLoad)
                    {
                        ...
                        {
                            Stream stream = GetType().Assembly.GetManifestResourceStream(String.Format("MyFightTracker.Views.{0}.{1}.html", this.Controller, this.View));
                            if (stream != null)
            ...
            if (!(IncludeMetadata("fields")))
                ...
                if ((_fieldFilter != null) && (_fieldFilter.Length > 0))
                {
                    List<DataField> newFields = new List<DataField>();
                    List<int> newFieldMap = new List<int>();
                    int index = 0;
                    foreach (DataField f in Fields)
                    	if (f.IsPrimaryKey || IncludeField(f.Name))
                        {
                            newFields.Add(f);
                    ...
                    if (newFields.Count != Fields.Count)
                    {
                        ...
                        {
                            object[] r = new object[newFields.Count];
                            index = 0;
                            foreach (int oldIndex in newFieldMap)
