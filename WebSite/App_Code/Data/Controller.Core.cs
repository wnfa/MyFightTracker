...
#pragma warning restore 0618
            ...
            if (o != null)
            	if (o.GetType() == typeof(List<object[]>))
                	...
                	if ((o is Array) && (o.GetType().GetElementType() == typeof(object)))
                    {
                    	...
                    	if (o is DateTime)
                        	return ((DateTime)(o)).ToString("yyyy-MM-ddTHH\\:mm\\:ss.fff");
            return o;
        }
        ...
        ViewPage IDataController.GetPage(string controller, string view, PageRequest request)
        {
            ...
            if ((page.FieldFilter != null) && ((page.FieldFilter.Length > 0) && (Config.SelectSingleNode("/c:dataController/c:businessRules/c:rule[@commandName=\'Select\']") != null)))
            	page.FieldFilter = null;
        ...
        public void PopulateManyToManyField(ViewPage page, DataField field, string primaryKeyField)
        {
            ...
            if (_junctionTableMap == null)
            {
                ...
                if (page.Rows.Count > 0)
                {
                    ...
                    string targetForeignKey1;
                    string targetForeignKey2;
        ...
        public virtual Stream GetDataControllerStream(string controller)
        {
            ...
            byte[] data = ApplicationServices.Current.ReadSiteContentBytes(("sys/controllers%/" 
                            + (controller + ".xml")));
            if (requiresUser)
            	context.User = null;
            if (data == null)
        ...
        protected virtual bool ValidateViewAccess(string controller, string view, string access)
        {
            ...
            if (AllowPublicAccess || (context.Request.Params["_validationKey"] == BlobAdapter.ValidationKey))
            	return true;
            if ((controller == ApplicationServices.SiteContentControllerName) && !(UserIsInRole(ApplicationServices.SiteContentEditors)))
            	return false;
            bool allow = true;
